...
void CDataParser::SetUInt64(uint64 val)
{
	*reinterpret_cast<uint64*>(pBuffLink + offset) = htobe64(val);
	offset += sizeof(uint64);
}

uint64 CDataParser::GetUInt64()
{
	uint64 val = *reinterpret_cast<uint64*>(pBuffLink + offset);
	offset += sizeof(uint64);
